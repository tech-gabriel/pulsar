...
export default function HistoricoPage() {
  ...
  // Última leitura para exibir variáveis atuais
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      {/* Header */}
      <header className="bg-pulsar-950 text-white px-6 py-4 flex items-center gap-4 shadow">
        <button
          onClick={() => navigate(-1)}
          className="text-pulsar-300 hover:text-white transition-colors flex items-center gap-2"
        >
          <ArrowLeft size={20} />
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <BarChart3 size={18} className="text-pulsar-400" />
            <h1
          ...
          {state?.regiaoNome && (
            <p className="text-xs text-pulsar-300 mt-0.5">Região: {state.regiaoNome}</p>
          )}
        </div>
        <span className="text-xs text-pulsar-300">Últimas 24h</span>
      </header>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-6 flex flex-col gap-6">
        ...
        {semDados && (
          <div className="flex flex-col items-center justify-center h-60 gap-3 text-slate-400">
            ...
            <p className="text-sm">São necessárias pelo menos 2 leituras para exibir o gráfico.</p>
          </div>
        )}

        {/* Variáveis climáticas atuais */}
        {ultima && !carregando && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div key={label} className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${cor}15` }}>
                  <Icon size={18} style={{ color: cor }} />
                </div>
                <div>
                  <p className="text-xs text-slate-500">{label}</p>
                  <p className="font-mono text-sm font-semibold text-slate-800">
                    {valor.toFixed(1)} <span className="text-slate-400 font-normal text-xs">{unidade}</span>
                  </p>
                </div>
              </div>
            ))}
        ...
        {dados.length >= 2 && (
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-slate-700" style={{ fontFamily: 'var(--font-heading)' }}>
                Score de Perigo
              </h2>
              {ultima?.score && (
        ...
        {dados.length >= 2 && (
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <h2 className="text-sm font-semibold text-slate-700 mb-4" style={{ fontFamily: 'var(--font-heading)' }}>
